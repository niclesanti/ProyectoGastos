...
- **Resúmenes**: Generated automatically by `ResumenScheduler`.
...
## Tool Usage for Java/Spring
- **Compilation**: Whenever you modify the code, run `cd backend` and `./mvnw clean compile` to ensure there are no syntax errors.
- **Migrations**: Before suggesting changes to Entities, check the `src/main/resources/db/migration` folder to avoid duplicating Flyway versions.
- **Tests**: If you create new financial logic (e.g., installment calculations), you must create and run a unit test using JUnit 5.